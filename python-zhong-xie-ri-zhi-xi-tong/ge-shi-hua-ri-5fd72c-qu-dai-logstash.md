...
直接将python的日志文件格式转成es所需要的json格式
一个好用的库


es可能出现 `org.elasticsearch.index.mapper.MapperParsingException: object mapping for [process] tried to parse field [process] as object, but found a concrete value`
的错误,process的mapping的格式错误,我直接把process从日志里删除,点进去源码
```python
'''
